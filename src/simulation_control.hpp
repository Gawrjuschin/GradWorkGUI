...
public slots:
  /**
   * @brief onStart - слот запуска симуляции.
   * Устанавливает флаг выполнения, который используется в деструкторе.
   * Вызывает соответствующий метод реализации симуляции.
   */
  void onStart() {
    m_busy_flag_ = true;
    emit sigStarted();
  ...
  /**
   * @brief onPause - слот паузы.
   * Вызывает соответствующий метод реализации симуляции.
   */
  void onPause() { p_worker->pause(); }
  /**
   * @brief onResume - слот возобновления.
   * Вызывает соответствующий метод реализации симуляции.
   */
  void onResume() { p_worker->resume(); }
  /**
   * @brief onStop - слот остановки.
   * Вызывает соответствующий метод реализации симуляции.
   */
