...
public:
    ...
    PointsData()
    : m_points_experimantal{QVector<QPointF>(kPointsCount),QVector<QPointF>(kPointsCount),QVector<QPointF>(kPointsCount),QVector<QPointF>(kPointsCount),QVector<QPointF>(kPointsCount),QVector<QPointF>(kPointsCount)}
    , m_points_approximation{QVector<QPointF>(kPointsCount),QVector<QPointF>(kPointsCount),QVector<QPointF>(kPointsCount),QVector<QPointF>(kPointsCount),QVector<QPointF>(kPointsCount),QVector<QPointF>(kPointsCount)}
    {
    ...
    void ReplacePoints(int graph, QVector<QPointF>&& points)
    {
        ...
        {
            std::copy(std::begin(m_points_experimantal.at(graph)), std::end(m_points_experimantal.at(graph)), std::begin(m_points_approximation.at(graph)));
        }

        // Расчёт границ
        {
            const auto [max, min] = std::minmax_element(std::cbegin(m_points_experimantal.at(graph)), std::cend(m_points_experimantal.at(graph)), [](const QPointF& lhs, const QPointF& rhs){ return lhs.x() < rhs.x(); });
