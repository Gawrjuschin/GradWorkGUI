    ...
    , p_start_btn(new QPushButton(tr("&Start"), this))
{
    ...
    // Настройка виджетов параметров системы
    {
        ...
        auto*  load_desc = new QLabel(QString(tr("[%1;  %2]"))
                                         .arg(InputData::min_load)
                                         .arg(InputData::max_load), this);
        ...
        p_parameter_desc->setText(QString(tr("[%1;  %2]"))
                                      .arg(QString().setNum(p_mu_input->value() * p_ch_input->value() * InputData::min_load, 'g', 4))
                                      .arg(QString().setNum(p_mu_input->value() * p_ch_input->value() * InputData::max_load, 'g', 4)));
        p_requests_desc->setText(QString(tr("[%1;  %2]"))
                                     .arg(QString().setNum(1/(p_mu_input->value() * p_ch_input->value() * InputData::max_load), 'g', 4))
                                     .arg(QString().setNum(1/(p_mu_input->value() * p_ch_input->value() * InputData::min_load), 'g', 4)));
