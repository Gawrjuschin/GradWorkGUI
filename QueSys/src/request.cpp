...
std::ostream& operator<<(std::ostream& os, const Request& request) {
  return os << std::setiosflags(std::ios::fixed) << std::setprecision(6)
            << request.number << '\t' << request.type << '\t'
            << request.arrive_time << '\t' << request.queue_number << '\t'
            << request.ch_number << '\t' << request.start_time << '\t'
            << request.serve_time << '\t' << request.wait_time << '\t'
...
std::istream& operator>>(std::istream& is, Request& request) {
  Request req{};
  if (is >> req.number && is >> req.type && is >> req.arrive_time &&
      is >> req.queue_number && is >> req.ch_number && is >> req.start_time &&
      is >> req.serve_time && is >> req.wait_time && is >> req.serve_end) {
    request = req;
  } else {
    is.setstate(std::ios::badbit);
  }
